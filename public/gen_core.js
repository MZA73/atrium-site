...
function rosace(cx,cy,s){doc.setDrawColor.apply(doc,C_OR);doc.setLineWidth(0.4);for(var i=0;i<8;i++){var a=i*Math.PI/4;doc.line(cx+Math.cos(a)*s*0.3,cy+Math.sin(a)*s*0.3,cx+Math.cos(a)*s,cy+Math.sin(a)*s);}doc.setFillColor.apply(doc,C_OR);doc.circle(cx,cy,1.2,'F');for(var j=0;j<4;j++){var b=j*Math.PI/2,lx=cx+Math.cos(b)*s,ly=cy+Math.sin(b)*s;doc.triangle(lx-1.2,ly,lx,ly-1.2,lx+1.2,ly,'F');doc.triangle(lx-1.2,ly,lx,ly+1.2,lx+1.2,ly,'F');}doc.setDrawColor.apply(doc,C_OR_SOMBRE);doc.setLineWidth(0.2);doc.circle(cx,cy,s+2,'S');}
function seal(cx,cy,r){doc.setDrawColor.apply(doc,C_OR);doc.setLineWidth(0.5);doc.circle(cx,cy,r,'S');doc.setDrawColor.apply(doc,C_OR_SOMBRE);doc.setLineWidth(0.2);doc.circle(cx,cy,r-1.5,'S');doc.setFillColor.apply(doc,C_OR);doc.triangle(cx-3,cy,cx,cy-3,cx+3,cy,'F');doc.triangle(cx-3,cy,cx,cy+3,cx+3,cy,'F');doc.setTextColor.apply(doc,C_OR_SOMBRE);doc.setFont('times','normal');doc.setFontSize(5);doc.text('LE TEMPLE',cx,cy-r+3.5,{align:'center'});doc.text('CPI 5904',cx,cy+r-2,{align:'center'});}
...
function cartouche(x,y,w,h,o){doc.setDrawColor.apply(doc,C_OR);doc.setLineWidth(0.5);doc.rect(x,y,w,h,'S');doc.setDrawColor.apply(doc,C_OR_SOMBRE);doc.setLineWidth(0.15);doc.rect(x+1.5,y+1.5,w-3,h-3,'S');doc.setFillColor.apply(doc,C_NOIR);doc.rect(x+1.5,y+1.5,w-3,11,'F');doc.setDrawColor.apply(doc,C_OR);doc.setLineWidth(0.2);doc.line(x+4,y+11,x+w-4,y+11);doc.setTextColor.apply(doc,C_OR_ECLAT);doc.setFont('times','normal');doc.setFontSize(9.5);doc.text(o.titre,x+w/2,y+7.5,{align:'center'});doc.setFont('times','italic');doc.setFontSize(7);doc.setTextColor.apply(doc,C_OR_SOMBRE);doc.text(o.sous,x+w/2,y+10,{align:'center'});doc.setTextColor.apply(doc,C_FILI);doc.setFont('times','italic');doc.setFontSize(9);(o.fil||[]).forEach(function(l,i){doc.text(l,x+w/2,y+24+i*4.6,{align:'center'});});doc.setDrawColor.apply(doc,C_OR_SOMBRE);doc.setLineWidth(0.2);doc.line(x+6,y+h-15,x+w-6,y+h-15);doc.setTextColor.apply(doc,C_NOIR);doc.setFont('times','normal');doc.setFontSize(9.5);doc.text(o.nom||'',x+w/2,y+h-9,{align:'center'});doc.setTextColor.apply(doc,C_OR_SOMBRE);doc.setFont('times','italic');doc.setFontSize(8);doc.text(o.qual||'',x+w/2,y+h-4.5,{align:'center'});}
...
cartouche(MARGIN_L+cw+gap,y,cw,ch,{titre:'LE LOCATAIRE',sous:'Locataire',fil:['Lu et approuve','bon pour bail'],nom:'',qual:(prof==='coloc'?'Tous colocataires':'Le Locataire')});
cartouche(MARGIN_L+2*(cw+gap),y,cw,ch,{titre:'LE MANDATAIRE',sous:'Le Temple - ATRIUM',fil:['(Cachet + sceau','electronique)'],nom:'M. ZAZOUA Mohammed',qual:'Gerant'});
...
y=chk(y+4,54);
cartouche(MARGIN_L,y,cw,ch,{titre:'LA CAUTION',sous:'Caution solidaire',fil:['Mention art. 2297','puis Lu et approuve'],nom:cnom,qual:'Caution'});
cartouche(MARGIN_L+cw+gap,y,cw,ch,{titre:'LE BAILLEUR',sous:'Creancier garanti',fil:['Lu et approuve'],nom:'',qual:'Le Bailleur'});
cartouche(MARGIN_L+2*(cw+gap),y,cw,ch,{titre:'LE MANDATAIRE',sous:'Le Temple',fil:['(Signature et cachet)'],nom:'M. ZAZOUA Mohammed',qual:'Gerant'});
})(_cs[ci],ci,_cs.length);}}
// footers
var np=doc.getNumberOfPages();for(var i=2;i<=np;i++){doc.setPage(i);footIn(i-1);}
return doc;
}
